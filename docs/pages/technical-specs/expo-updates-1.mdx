...
An _update_ is defined as one or both of the following:
- a [_manifest_](#manifest-response) together with the assets referenced inside the manifest.
- a [_message_](#message-response) directive.

Expo Updates is a protocol for assembling and delivering updates to apps running on multiple platforms.

An app running a conformant Expo Updates client library MUST load the most recent _manifest_ saved in the client library's cache possibly after filtering by the contents of the manifest's [_metadata_](#manifest-body).

The following describes how a conformant Expo Updates client library MUST retrieve a new update from a conformant server:

1. The client library will make a [request](#update-request) for the most recent update, with constraints specified in the headers.
2. If a new update is downloaded, the client library will process its parts:
    - For a _manifest_ part, the client library will proceed to make additional requests to download and store any missing assets specified in the manifest. This is considered a new _manifest_.
    - For a _message_ part, the client library will consume the message depending on the message type and update its local state accordingly.
...
1. `expo-protocol-version: 1`, to specify version 1 of this expo-updates specicifcation.
...
A conformant server MUST return a response structured in at least one of two ways. A conformant server MAY support either or both response structures, and when an unsupported response structure is requested the server SHOULD respond with an HTTP `406` error status. A server that wishes to respond with an incompatible response for the requested protocol version SHOULD respond with an HTTP `404` error status instead.

- For a response with `content-type: application/json` or `content-type: application/expo+json`, the [common response headers](#common-response-headers) and  [other response headers](#other-response-headers) MUST be sent in the response headers and the [manifest body](#manifest-body) MUST be sent in the response body. This format of response does not support update parts and therefore does not support _message_ directives, and SHOULD respond with an HTTP `404` error status when the most recent update to be served is not a _manifest_.
...
- `expo-server-defined-headers` is an [Expo SFV](expo-sfv-0.mdx) dictionary. It defines headers that a client library MUST store until overwritten by a newer dictionary, and they MUST be included in every subsequent [update request](#update-request).
...
- `content-type` MUST be determined by _proactive negotiation_ as defined in [RFC 7231](https://tools.ietf.org/html/rfc7231#section-3.4.1). Since the client library is [required](#update-request) to send an `accept` header with each manifest request, this will always be either `application/expo+json`, `application/json`; otherwise the request would return a `406` error.
...
3. OPTIONAL `"message"` part:
    - MUST have part header `content-disposition: inline; name="message"`.
    - MUST have part header `content-type: application/json` or `application/expo+json`.
    - SHOULD have part header `expo-signature` as defined in [other response headers](#other-response-headers) if code signing is being used.
    - The [message body](#message-body) MUST be sent in the part body.
...
Defined as JSON conforming to both the following `Message` definition expressed in [TypeScript](https://www.typescriptlang.org/) and the detailed descriptions for each field:
...
- `messageType`: The type of message directive.
- `messagePayload`: MAY contain any extra information specific to the `messageType`.

A conformant client library and server SHOULD specify and implement message types specific to the needs of the application. For example, Expo Application Services makes use of two message types: `noUpdateAvailable` (directs the expo-updates library to proceed as normal) and `rollbackToEmbedded` (directs the expo-updates library to use the update embedded in the host application instead of any other downloaded updates).
...
Expo Updates supports code signing the manifest and message. Code signing the manifest also transitively signs the assets since their hashes are present in the manifest and verified by a conformant client. A conformant client MAY request the manifest or message be signed using a private key, and then MUST verify the signature of the manifest or message using the corresponding code signing certificate before it is used or any corresponding manifest assets are downloaded. The client MUST verify that the signing certificate is either a self-signed, trusted root certificate or is in a certificate chain signed by a trusted root certificate. In either case, the root certificate MUST be embedded in the application or device's operating system.

...
- a [_directive_](#directive-response).
...
2. If a new update is downloaded, the client library will process its parts:
    ...
    - For a _directive_ part, the client library will consume the directive depending on the directive type and update its local state accordingly.
...
- For a response with `content-type: application/json` or `content-type: application/expo+json`, the [common response headers](#common-response-headers) and  [other response headers](#other-response-headers) MUST be sent in the response headers and the [manifest body](#manifest-body) MUST be sent in the response body. This format of response does not support update parts and therefore does not support _directive_, and SHOULD respond with an HTTP `404` error status when the most recent update to be served is not a _manifest_.
...
type Directive = {
  ...
  directivePayload?: { [key: string]: any };
  [key: string]: any;
};
```

- `directiveType`: The type of directive directive.
- `directivePayload`: MAY contain any extra information specific to the `directiveType`.

A conformant client library and server SHOULD specify and implement directive types specific to the needs of the application. For example, Expo Application Services makes use of two directive types: `noUpdateAvailable` (directs the expo-updates library to proceed as normal) and `rollbackToEmbedded` (directs the expo-updates library to use the update embedded in the host application instead of any other downloaded updates).

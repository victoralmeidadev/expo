...
class RemoteLoaderTest {
  ...
  @Test
  fun testRemoteLoader_RollbackMessage() {
    val updateMessage = UpdateMessage.RollbackToEmbeddedUpdateMessage(commitTime = Date(), signingInfo = null)
    every { mockFileDownloader.downloadRemoteUpdate(any(), any(), any(), any()) } answers {
      val callback = arg<FileDownloader.RemoteUpdateDownloadCallback>(3)
      callback.onSuccess(
        UpdateResponse(
          ...
          messageUpdateResponsePart = UpdateResponsePart.MessageUpdateResponsePart(updateMessage)
        )
      )
    }
    ...
    verify { mockCallback.onSuccess(Loader.LoaderResult(null, updateMessage)) }
    verify(exactly = 0) { mockCallback.onFailure(any()) }
    verify(exactly = 0) { mockFileDownloader.downloadAsset(any(), any(), any(), any(), any()) }
